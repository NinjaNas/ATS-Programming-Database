const {
  indexController,
} = require("../../../server/controllers/session/questionnaire/");
...
it("should send a status of 200 when sending a value", async () => {
  ...
  // Select questionnaire table
  await mPool.query.mockResolvedValueOnce([[{ test: "value" }], []]);
  // Call controller
  await indexController(req, res);
  // Call to select questionnaire table
  expect(mPool.query).toHaveBeenCalledWith("SELECT * FROM sel_questionnaire;");
  // Actual send return
