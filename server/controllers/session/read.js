...
async function readController(req, res) {
  ...
  if (key == 0) {
      await pool.query("SELECT * FROM session WHERE id=?;", [tag])
      .then((table) => {
      ...
      await pool.query("SELECT * FROM session WHERE user_id=?;", [tag])
      .then((table) => {
          res.send(table[0]);
        })
      .catch((err) => {
      ...
      await pool.query("SELECT * FROM session WHERE status=?;", ["0"])
      .then((table) => {
          res.send(table[0]);
        })
      .catch((err) => {
